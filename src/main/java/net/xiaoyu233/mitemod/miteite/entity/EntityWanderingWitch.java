...
public class EntityWanderingWitch extends EntityWitch {
    ...
    public boolean getAggressive() {
        ...
        if (iteWitch.getSummon_wolf_countdown() > 0) {
            iteWitch.setSummon_wolf_countdown(iteWitch.getSummon_wolf_countdown() - 1);
            if (iteWitch.getSummon_wolf_countdown() == 0) {
                iteWitch.setHas_summoned_wolves(this.summonWolvesITE() > 0 || iteWitch.isHas_summoned_wolves());
            }
        } else if (this.getLastHarmingEntity() instanceof EntityPlayer) {
            iteWitch.setSummon_wolf_target((EntityLivingBase)this.getLastHarmingEntity());
            iteWitch.setSummon_wolf_countdown(160);
        }
        return true;
    }
    ...
    public boolean getCanSpawnHere(boolean perform_light_check) {
        ...
        if (chanceSpawn){
            ITEWorld iteWorld = this.worldObj;
            if (this.getWorld().isOverworld()){
                return super.getCanSpawnHere(perform_light_check) && iteWorld.getDayOfOverworld() > Configs.GameMechanics.MobSpawning.WANDERING_WITCH_SPAWN_LIMIT_DAY_OVERWORLD.get();
            }else {
                return super.getCanSpawnHere(perform_light_check) && iteWorld.getDayOfOverworld() > Configs.GameMechanics.MobSpawning.WANDERING_WITCH_SPAWN_LIMIT_DAY_OTHER.get();
            }
        }
        return false;
    }
    ...
    public void onLivingUpdate() {
        ...
        if (this.onServer()) {
            ...
            if (this.fleeing){
                if (this.throwingPotionCooldown <= 0) {
                    closestPlayerToEntity1.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, rand.nextInt(30) + 20,0));
                    this.attackEntityWithRangedAttack(closestPlayerToEntity1,1.0f);
                    this.summonWolvesITE();
    ...
    public int summonWolvesITE() {
        EntityLivingBase summon_wolf_target = this.getSummon_wolf_target();
        if (summon_wolf_target != null){
            EntityLivingBase target = (EntityLivingBase)this.worldObj.getEntityByID(summon_wolf_target.entityId);
            if (target != null && !target.isDead) {
                ...
                for(int attempts = 0; attempts < batCounts; ++attempts) {
                    ...
                    this.aliveBatsCount++;
                    PathNavigate navigator = bat.getNavigator();
    ...
    protected void applyEntityAttributes() {
        ...
        this.setEntityAttribute(SharedMonsterAttributes.maxHealth, 80.0);
