...
public class ItemArmorLiar extends Item implements IDamageableItem {
    @Liar
    ...
    public void addInformation(ItemStack item_stack, EntityHuman player, List info, boolean extended_info, Slot slot) {
        if (extended_info) {
            ...
            info.add(EnumChatFormat.j + Translator.getFormatted("item.tooltip.protectionBonus", new Object[]{StringHelper.formatFloat(protection, decimal_places, decimal_places)}));
        }
        if (item_stack.p()) {
            int toolLevel = item_stack.q().e("tool_level");
            if (item_stack.q().b("tool_level")) {
                info.add("装备等级:" + toolLevel);
            }
            if (item_stack.q().b("tool_exp")) {
                info.add("装备经验" + EnumChatFormat.p + item_stack.q().e("tool_exp") + "/" + this.getExpReqForLevel(
    ...
    @Stealing
    private float getProtectionAfterDamageFactor(ItemStack item_stack, EntityLiving owner) {
        return 0;
    }
