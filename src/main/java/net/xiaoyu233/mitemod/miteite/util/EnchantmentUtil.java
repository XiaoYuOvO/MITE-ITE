...
public class EnchantmentUtil {
    public static void removeEnchantmentsFromMapThatConflict(Map<Integer,EnchantmentData> map, ArrayList<EnchantmentData> enchantments, Item item) {
        for (EnchantmentData o : enchantments) {
            ...
            while (iterator.hasNext()) {
                ...
                if (enchantment instanceof EnchantmentDamage) {
                    if (!ItemUtil.isDamageEnchantmentCompatibleWith(enchantment, Enchantment.get(id), item)) {
                        iterator.remove();
                    }
                } else {
    ...
    public static List<EnchantmentData> buildEnchantmentList(Random random, ItemStack item_stack, int enchantment_levels,int maxTotalLevel,int maxEnchantmentNum) {
        ...
        if (enchantability <= 0) {
            ...
            List<EnchantmentData> enchantments_for_item_shuffled = new ArrayList<>();
